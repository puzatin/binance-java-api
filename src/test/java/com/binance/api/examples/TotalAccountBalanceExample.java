...
public class TotalAccountBalanceExample {
    ...
    public static void main(String[] args) {
        ...
        double totalBalanceInUSDT = totalBalanceInBTC * Double.parseDouble(client.getAvgPrice("BTCUSDT").getAveragePrice());
        ...
        for (AssetBalance balance : account.getBalances()) {
            ...
            if (free + locked != 0) {
                if (Util.isFiatCurrency(balance.getAsset())) {
                    double price = Double.parseDouble(client.getAvgPrice(tickerRev).getAveragePrice());
                    double amount = (free + locked) / price;
                    totalBalance += amount;
                } else {
                    double price = Double.parseDouble(client.getAvgPrice(ticker).getAveragePrice());
